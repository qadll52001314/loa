...
void ScriptMgr::Unload()
{
    ...
    SCR_CLEAR(CapitalCityScript);
    SCR_CLEAR(ResourcePointScript);
...
ResourcePoint* ScriptMgr::CreateResourcePoint(uint32 scriptId)
{
    GET_SCRIPT_RET(ResourcePointScript, scriptId, tmpscript, NULL);
    return tmpscript->GetResourcePoint();
}

CapitalCity* ScriptMgr::CreateCapitalCity(uint32 scriptId)
{
    GET_SCRIPT_RET(CapitalCityScript, scriptId, tmpscript, NULL);
    return tmpscript->GetCapitalCity();
}
...
CapitalCityScript::CapitalCityScript(const char* name)
    : ScriptObject(name)
{
    ScriptRegistry<CapitalCityScript>::AddScript(this);
}

ResourcePointScript::ResourcePointScript(const char* name)
    : ScriptObject(name)
{
    ScriptRegistry<ResourcePointScript>::AddScript(this);
}
...
template class ScriptRegistry<CapitalCityScript>;
template class ScriptRegistry<ResourcePointScript>;
