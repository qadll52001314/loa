...
uint32 CompoundMgr::CanCompoundRecipe(CompoundReagent reagent, const RecipeCompound* compound, const Player* player) const
{
    uint32 point = 0;
...
CompoundReagent CompoundMgr::StackReagents(const CompoundReagentItem& reagent) const
{
    CompoundReagent reag;
    for (uint8 i = 0; i != MAX_COMPOUND_REAGENT; ++i)
    {
        reag.reagent[i] = reagent.reagent[i] ? reagent.reagent[i]->GetEntry() : 0;
        reag.count[i] = reagent.reagent[i] ? reagent.reagent[i]->GetCount() : 0;
    }
    ...
    for (uint8 i = MAX_COMPOUND_REAGENT - 1; i != 0; --i)
    {
        if (reag.reagent[i])
        {
            for (uint8 j = 0; j < i; ++j)
            {
                if (!reag.reagent[j])
                {
                    reag.reagent[j] = reag.reagent[i];
                    reag.count[j] = reag.count[i];
                    reag.reagent[i] = 0;
                    reag.count[i] = 0;
                    break;
                }
    ...
    for (uint8 i = 0; i != MAX_COMPOUND_REAGENT; ++i)
    {
        if (reag.reagent[i])
        {
            ...
            {
                if (reag.reagent[i] == reag.reagent[j])
                {
                    reag.count[i] += reag.count[j];
                    reag.reagent[j] = 0;
                    reag.count[j] = 0;
                }
            }
...
const RecipeCompound* CompoundMgr::FindRecipeCompound(const Player* player, CompoundReagent reagent) const
{
...
const GemCompound* CompoundMgr::FindGemCompound(uint32 rawStone, uint32 rawCount) const
{
    GemCompoundBounds bound = GetGemCompoundBounds(rawStone);
    if (bound.first == bound.second)
        return NULL;
...
const EnchantCompound* CompoundMgr::FindEnchantCompound(Player* player, const ItemTemplate* item, CompoundReagent reagent) const
{
...
uint32 CompoundMgr::CanCompoundEnchant(const ItemTemplate* item, CompoundReagent reagent, const EnchantCompound* compound, const Player* player) const
{
    uint32 point = 0;
...
const SocketCompound* CompoundMgr::FindSocketCompound(const Player* player, const ItemTemplate* item, CompoundReagent reagent) const
{
...
uint32 CompoundMgr::CanCompoundSocket(const ItemTemplate* item, CompoundReagent reagent, const SocketCompound* compound) const
{
...
uint32 CompoundMgr::CalculateRecycleAmount(CompoundReagent& reagent) const
{
    uint32 amount = 0;
    for (uint8 i = 0; i != MAX_COMPOUND_REAGENT; ++i)
    {
...
void CompoundMgr::ProcessCompound(Player* player, CompoundReagentItem reagent) const
{
    CompoundProcessor::Process(player, reagent);
}
