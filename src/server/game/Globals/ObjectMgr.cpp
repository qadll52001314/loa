...
void ObjectMgr::CheckCreatureTemplate(CreatureTemplate const* cInfo)
{
    ...
    const_cast<CreatureTemplate*>(cInfo)->ModDamage *= Creature::_GetDamageMod(cInfo->rank, cInfo->expansion);
}
...
void ObjectMgr::LoadGossipText()
{
    ...
    QueryResult result = WorldDatabase.Query("SELECT ID, "
        "BroadcastTextID0, prob0, "
        "BroadcastTextID1, prob1, "
        "BroadcastTextID2, prob2, "
        "BroadcastTextID3, prob3, "
        "BroadcastTextID4, prob4, "
        "BroadcastTextID5, prob5, "
        "BroadcastTextID6, prob6, "
        "BroadcastTextID7, prob7 "
        "FROM npc_text");
...
uint32 ObjectMgr::GetXPForSupremacyLevel(uint32 level) const
{
    if (level < _playerXPperSupremacyLevel.size())
        return _playerXPperSupremacyLevel[level];
    return 0;
}
...
void ObjectMgr::AddSpellToTrainer(uint32 entry, uint32 spell, uint32 spellCost, uint32 reqSkill, uint32 reqSkillValue, uint32 reqLevel, uint32 reqCityRank)
{
    ...
    trainerSpell.reqCityRank   = reqCityRank;
...
void ObjectMgr::LoadTrainerSpell()
{
    ...
    QueryResult result = WorldDatabase.Query("SELECT b.entry, a.spell, a.spellcost, a.reqskill, a.reqskillvalue, a.reqlevel, a.reqcityrank FROM npc_trainer AS a "
    ...
    do
    {
        ...
        uint32 reqCityRank = fields[6].GetUInt32();

        AddSpellToTrainer(entry, spell, spellCost, reqSkill, reqSkillValue, reqLevel, reqCityRank);
...
int ObjectMgr::LoadReferenceVendor(int32 vendor, int32 item, std::set<uint32> *skip_vendors)
{
    ...
    do
    {
        ...
        else
        {
            ...
            uint32 ReqCityRank = fields[4].GetUInt32();
            ...
            vList.AddItem(item_id, maxcount, incrtime, ExtendedCost, ReqCityRank);
            ++count;
        }
    } while (result->NextRow());
...
void ObjectMgr::LoadVendors()
{
    ...
    QueryResult result = WorldDatabase.Query("SELECT entry, item, maxcount, incrtime, ExtendedCost, ReqCityRank FROM npc_vendor ORDER BY entry, slot ASC");
    if (!result)
    {
    ...
    do
    {
        ...
        else
        {
            ...
            uint32 ReqCityRank  = fields[5].GetUInt32();
...
void ObjectMgr::LoadGossipMenuItems()
{
    ...
    QueryResult result = WorldDatabase.Query(
        ...
        "SELECT menu_id, id, option_icon, OptionBroadcastTextID, option_id, npc_option_npcflag, action_menu_id, action_poi_id, box_coded, box_money, BoxBroadcastTextID, SingleTimeCheck "
    ...
    do
    {
        ...
        gMenuItem.OptionBroadcastTextId = fields[3].GetUInt32();
        ...
        gMenuItem.SingleTimeCheck       = fields[11].GetBool();
...
void ObjectMgr::AddVendorItem(uint32 entry, uint32 item, int32 maxcount, uint32 incrtime, uint32 extendedCost, uint32 reqCityRank, bool persist /*= true*/)
{
    VendorItemData& vList = _cacheVendorItemStore[entry];
    vList.AddItem(item, maxcount, incrtime, extendedCost, reqCityRank);
...
std::string ObjectMgr::GetServerMessage(uint32 id, ...)
{
    ServerMessageContainer::iterator itr = _serverMessageStore.find(id);
    if (itr != _serverMessageStore.end())
    {
        const char* message = itr->second.c_str();
        va_list ap;
        char szStr[1024];
        va_start(ap, id);
        vsnprintf(szStr, 1024, message, ap);
        va_end(ap);
        return std::string(szStr);
    }
    return "<error retriving string>";
}

bool ObjectMgr::LoadServerMesssages()
{
    uint32 oldMSTime = getMSTime();

    _serverMessageStore.clear(); // for reload case

    QueryResult result = WorldDatabase.Query("SELECT id, content FROM server_messages");
    if (!result)
    {
        TC_LOG_ERROR("server.loading", ">> Loaded 0 server messages. DB table `server_messages` is empty.");
        return false;
    }
    ...
    {
        Field* fields = result->Fetch();
        _serverMessageStore[fields[0].GetUInt32()] = fields[1].GetString();
    } while (result->NextRow());
