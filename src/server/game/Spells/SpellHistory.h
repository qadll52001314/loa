...
public:
    ...
    void ModifyCooldowns(Predicate predicate, int32 cooldownModMs)
    {
        CooldownStorageType vCopy(_spellCooldowns);
        for (auto itr = vCopy.begin(); itr != vCopy.end();)
        {
            if (predicate(itr))
                ModifyCooldown(itr->first, cooldownModMs);
            else
                ++itr;
        }
    }
