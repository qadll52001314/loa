...
pEffect SpellEffects[TOTAL_SPELL_EFFECTS]=
{
    ...
    &Spell::EffectPowerDamage,                              //165 SPELL_EFFECT_POWER_DAMAGE
    &Spell::EffectPowerHeal,                                //166 SPELL_EFFECT_POWER_HEAL
    &Spell::EffectComprehendSpell,                          //167 SPELL_EFFECT_COMPREHEND_SPELL
...
void Spell::EffectPowerDamage(SpellEffIndex effIndex)
{
    // Note: this effect should only be used by players.
    if (effectHandleMode != SPELL_EFFECT_HANDLE_LAUNCH_TARGET)
        return;
    ...
    if (unitTarget && unitTarget->IsAlive())
    {
        ...
        switch (m_spellInfo->DmgClass)
        {
            case SPELL_DAMAGE_CLASS_MELEE:
                DamagePower = caster->GetTotalAttackPowerValue(BASE_ATTACK);
                break;
            case SPELL_DAMAGE_CLASS_RANGED:
                DamagePower = caster->GetTotalAttackPowerValue(RANGED_ATTACK);
                break;
            case SPELL_DAMAGE_CLASS_MAGIC:
                DamagePower = caster->SpellBaseDamageBonusDone(m_spellInfo->GetSchoolMask());
                break;
            default:
                return;
        }

        damage = CalculatePct(DamagePower, damage);
            ...
            case SPELLFAMILY_WARRIOR:
            {
                switch (m_spellInfo->Id)
                {
                    case 20252: // intercept
                        if (unitTarget)
                            damage += CalculatePct(damage, 100 - unitTarget->GetHealthPct() * m_spellInfo->Effects[2].CalcValue());
                        break;
                    case 23922: // shield bash
                        damage = CalculatePct(m_caster->GetShieldBlockValue(), m_spellInfo->Effects[1].CalcValue());
                        break;
                }
            }   break;
            case SPELLFAMILY_WARLOCK:
            ...
            case SPELLFAMILY_PALADIN:
            {
                switch (m_spellInfo->Id)
                {
                    case 20467: // Judgement of Command
                        if (unitTarget->HasAuraWithMechanic(1 << MECHANIC_STUN) || unitTarget->HasAuraWithMechanic(1 << MECHANIC_KNOCKOUT))
                            damage *= 2;
                        break;
                    case 53600:
                        damage = CalculatePct(caster->GetShieldBlockValue(), m_spellInfo->Effects[0].CalcValue());
                        break;
                    case 31804:
                        if (Aura* aura = unitTarget->GetAura(31803))
                            damage += CalculatePct(damage, aura->GetStackAmount() * 20);
                        break;
                }
            }   break;
            default:
        ...
        // randomize a little to look nicer.
        damage *= frand(0.8f, 1.2f);
...
void Spell::EffectPowerHeal(SpellEffIndex effIndex)
{
    if (effectHandleMode != SPELL_EFFECT_HANDLE_LAUNCH_TARGET)
        return;
    ...
    if (unitTarget && unitTarget->IsAlive() && damage >= 0)
    {
        ...
        {
            case 1: // Physical, by attack power or ranged attack power(max one).
                addhealth = std::max(caster->GetTotalAttackPowerValue(BASE_ATTACK), caster->GetTotalAttackPowerValue(RANGED_ATTACK));
                break;
            case 2: // Holy, by holy spell damage power
                addhealth = caster->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_HOLY);
                break;
            case 4: // Fire
                addhealth = caster->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_FIRE);
                break;
            case 8: // Nature
                addhealth = caster->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_NATURE);
                break;
            case 16: // Frost
                addhealth = caster->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_FROST);
                break;
            case 32: // Shadow
                addhealth = caster->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_SHADOW);
                break;
            case 64: // Arcane
                addhealth = caster->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_ARCANE);
                break;
            case 0: // Healing case
                addhealth = caster->SpellBaseHealingBonusDone(SPELL_SCHOOL_MASK_ALL);
                break;
            default:
                addhealth = caster->SpellBaseHealingBonusDone(SPELL_SCHOOL_MASK_ALL);
                break;
        ...
        addhealth = CalculatePct(addhealth, damage);
        addhealth = caster->SpellHealingBonusDone(unitTarget, m_spellInfo, addhealth, HEAL);
        addhealth = unitTarget->SpellHealingBonusTaken(caster, m_spellInfo, addhealth, HEAL);

        // for players add target based heal. 1.5s for 5% so 1s(and instant) = 3.33%
        //if (m_caster->GetTypeId() == TYPEID_PLAYER && unitTarget->GetTypeId() == TYPEID_PLAYER)
        //{
        //    addhealth += CalculatePct(unitTarget->GetMaxHealth(), 0.003333f * std::min((int)m_spellInfo->CalcCastTime(), 1000));
        //}

        addhealth *= frand(0.8f, 1.2f);
...
void Spell::EffectComprehendSpell(SpellEffIndex effIndex)
{
    ...
    //if (entryA)
    //    xCore->ComprehendSpell(m_caster->ToPlayer(), entryA);
    //if (entryB)
    //    xCore->ComprehendSpell(m_caster->ToPlayer(), entryB);
}
...
void Spell::EffectGrantBaseSpellSet(SpellEffIndex effIndex)
{
    ...
    //uint32 id = m_spellInfo->Effects[effIndex].MiscValue;

    //if (id)
    //    unitTarget->ToPlayer()->GrantBaseSpellSet(id);
}

enum WarCurrency
{
    WAR_CURRENCY_ALLIANCE           = 57347,
    WAR_CURRENCY_HORDE              = 57346
};

enum WarMailEntry
{
    WAR_MAIL_HORDE                  = 293,
    WAR_MAIL_ALLIANCE               = 294
};

enum WarMailSenderEntry
{
    WAR_MAIL_SENDER_HORDE           = 43297,
...
void Spell::EffectGrantCityResource(SpellEffIndex effIndex)
{
    ...
    if (Player* player = unitTarget->ToPlayer())
    {
        ...
        MailDraft draft = MailDraft(player->GetTeamId() ? WAR_MAIL_HORDE : WAR_MAIL_ALLIANCE);
        Item* item = Item::CreateItem(player->GetTeamId() ? WAR_CURRENCY_HORDE : WAR_CURRENCY_ALLIANCE, value, player);
        if (item)
        {
            item->SaveToDB(trans);
            draft.AddItem(item);
...
void Spell::EffectGrantCityMagicPower(SpellEffIndex effIndex)
{
    ...
    if (Player* player = unitTarget->ToPlayer())
    {
        SQLTransaction trans = CharacterDatabase.BeginTransaction();
        MailDraft draft = MailDraft(player->GetTeamId() ? WAR_MAIL_HORDE : WAR_MAIL_ALLIANCE);
        Item* item = Item::CreateItem(player->GetTeamId() ? WAR_CURRENCY_HORDE : WAR_CURRENCY_ALLIANCE, value * 2, player); // magic power gets 200% currency reward
        if (item)
        {
            item->SaveToDB(trans);
            draft.AddItem(item);
